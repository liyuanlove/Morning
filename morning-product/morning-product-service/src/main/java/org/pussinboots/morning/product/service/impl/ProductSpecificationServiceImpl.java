...
public class ProductSpecificationServiceImpl extends ServiceImpl<ProductSpecificationMapper, ProductSpecification> implements IProductSpecificationService {
	...
	@Override
	public ProductSpecificationDTO getByProductId(String productNumber, Integer status) {
		// 根据产品ID和类型状态查找类型列表
		List<KindVO> kindVOs = kindMapper.listByProductId(productNumber, status);
		...
		if (kindVOs.isEmpty()) {
			// 根据商品ID获取默认商品规格表
			ProductSpecification productSpecification = productSpecificationMapper.getByStatus(productNumber, status,
