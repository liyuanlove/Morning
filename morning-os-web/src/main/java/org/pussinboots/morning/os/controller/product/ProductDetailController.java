...
public class ProductDetailController extends BaseController {
	...
	public String item(Model model, @PathVariable("productNumber") String productNumber) {
		//商品冻结判断
		if(!productService.productIsForzen(productNumber)){
			return "/common/productfrozen";
		}
		// 根据商品编号查找商品信息
		try{
			...
			if(product != null) {
				...
				// 根据商品ID查找商品展示图片
				List<ProductImage> productImages = productImageService.listByProductId(product.getProductNumber(),
						ProductConstantEnum.PRODUCT_PICIMG_NUMBER.getValue(), StatusEnum.SHOW.getStatus());

				// 根据商品ID查找商品参数
				List<ProductParameter> productParameters = productParameterService.listByProductId(product.getProductNumber(),
						StatusEnum.SHOW.getStatus());

				// 根据商品ID查找商品类型列表以及商品规格列表
				ProductSpecificationDTO productSpecificationDTO = productSpecificationService
						.getByProductId(product.getProductNumber(), StatusEnum.SHOW.getStatus());
