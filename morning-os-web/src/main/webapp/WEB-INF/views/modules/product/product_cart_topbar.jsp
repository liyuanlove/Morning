...
<c:if test="${not empty cartVO}">
  <ul class="cart-list">
    <c:forEach items="${cartVO.shoppingCartVOs}" var="shoppingCart">
      <li>
        <div class="cart-item clearfix first"><a class="thumb" href="${ctx}/detail/${shoppingCart.productNumber}"><img alt="${shoppingCart.name}" src="${imageServer}${shoppingCart.picImg}" width="60" height="60"></a><a class="name" href="${ctx}/detail/${shoppingCart.productNumber}">${shoppingCart.name}
  ...
  <div class="cart-total clearfix"><span class="total">共 <em>${cartVO.totalNumber}</em> 件商品<span class="price"><em>${cartVO.totalPrice}</em>元</span></span><a href="${ctx}/cart/list" class="btn btn-primary btn-cart">去购物车结算</a></div>
</c:if>
